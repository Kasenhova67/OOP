...
{    public class Document : ISubject
    {
        ...
        private readonly List<TextComponent> _content = new();
        ...
        public void Notify(string message)
        {
            ...
            {
                observer.Update(message, Title);
            }

            _notificationService.TrackChange(Title, message, UserManager.Instance.GetCurrentUser()?.Name);
        }
        ...
        public void InsertText(int position, string text, string modifiedBy, string format = "Plain")
        {
            ...
            Notify($"Text inserted at position {position} by {modifiedBy}");
            _notificationService.TrackChange(Title, $"Text inserted at position {position}", modifiedBy);
            NotificationService.SendNotificationToSubscribers(
           Title,
           $"Inserted text at position {position}",
           modifiedBy);
        }

      
        public enum DisplayMode { Preview, Edit }

        public void Save(string modifiedBy)
        {
            _notificationService.CommitChanges(Title);
            Notify($"Document saved by {modifiedBy}");
        }
        ...
        public void DeleteText(int position, int length, string modifiedBy)
        {
            ...
            _undoStack.Push(command);
            _redoStack.Clear();
            Notify($"Deleted {length} characters from position {position} by {modifiedBy}");
            
            _notificationService.TrackChange(Title, $"Text deleted from position {position}", modifiedBy);
            NotificationService.SendNotificationToSubscribers(
           Title,
           $"Deleted text",
           modifiedBy);
        ...
        public void Undo(string modifiedBy)
        {
            if (_undoStack.Count > 0)
            {
                ...
                Notify($"Undo performed by {modifiedBy}");
                _notificationService.TrackChange(Title, $"Undo", modifiedBy);
                NotificationService.SendNotificationToSubscribers(
           Title,
           $"Undo",
           modifiedBy);
        ...
        public void Redo(string modifiedBy)
        {
            if (_redoStack.Count > 0)
            {
                ...
                command.Execute();
                _undoStack.Push(command);
                Notify("Redo performed by {modifiedBy}");
                _notificationService.TrackChange(Title, $"Redo", modifiedBy);
                NotificationService.SendNotificationToSubscribers(
          Title,
          $"redo",
          modifiedBy);
        ...
        public string GetFullText()
        {
            ...
            return sb.ToString();
               ...
               public void Copy(int start, int length, string modifiedBy)
        {
            string text = GetTextRange(start, length);
            if (!string.IsNullOrEmpty(text))
            {
                _clipboard = text;
                Notify($"Copied {length} characters to clipboard by {modifiedBy}");
                _notificationService.TrackChange(Title, $"Copied text", modifiedBy);
                NotificationService.SendNotificationToSubscribers(
          Title,
          $"Coppied",
          modifiedBy);
        ...
        public void Cut(int start, int length, string modifiedBy)
        {
            string text = GetTextRange(start, length);
            if (!string.IsNullOrEmpty(text))
            {
                _clipboard = text;
                DeleteText(start, length,modifiedBy);
                Notify($"Cut {length} characters to clipboard by {modifiedBy}");
                _notificationService.TrackChange(Title, $"Cut text", modifiedBy);
                NotificationService.SendNotificationToSubscribers(
         Title,
         $"Cut",
         modifiedBy);
        ...
        public void Paste(int position, string modifiedBy)
        {
            if (!string.IsNullOrEmpty(_clipboard))
            {
                InsertText(position, _clipboard, modifiedBy);
                Notify($"Pasted {_clipboard.Length} characters from clipboard");
                _notificationService.TrackChange(Title, $"Paste text", modifiedBy);
                NotificationService.SendNotificationToSubscribers(
         Title,
         $"Pasted",
         modifiedBy);
...
/*
        public void DeleteRange(int startIndex, int endIndex)
        {
            ...
            _notificationService.TrackChange(Title, $"Dele text", modifiedBy);
        }*/
        ...
        public void ApplyFormat(int start, int end, string formatType, string modifiedBy)
        {
            ...
            Notify($"Text was formated by {modifiedBy}");
            _notificationService.TrackChange(Title, $"Text was formated", modifiedBy);
            NotificationService.SendNotificationToSubscribers(
           Title,
           $"Text was formated",
           modifiedBy);
